...
/*   Updated: 2019/06/19 17:48:42 by jdurand-         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include <corewar.h>

static int	instr_ldi_inner(t_thread **th, int ***gr, int size1, int size2)
{
	int		reg;
	int		total;
	int		value1;
	...
	if (thread_check(th) < VALID_EMPTY || grid_check(gr) != VALID_FULL)
		return (BAD_PARAM);
	value1 = size1 == 4 ? read_in_grid(gr, (*th)->where + 1 + 1, 2)
		: read_in_grid(gr, (*th)->where + 1 + 1, size1);
	...
	reg = read_in_grid(gr, (*th)->where + 1 + 1 + size1 + size2, 1);
...
int			instr_ldi(t_thread **th, int ***gr)
{
	...
	free(tab);
	return (instr_ldi_inner(th, gr, size1, size2));
}
