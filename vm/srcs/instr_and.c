...
/*   Updated: 2019/06/17 16:54:11 by jdurand-         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include <corewar.h>

static int	instr_and_inner(t_thread **th, int ***gr, int size1, int size2)
{
	int		value1;
	int		value2;
	...
	if (thread_change_value_reg(th, read_in_grid(gr, (*th)->where
		...
		return (CALL_FAILED);
	if (thread_change_where(th, gr, (*th)->where + 1 + size1 + size2 + 1 + 1) != SUCCESS)
		return (CALL_FAILED);
...
int			instr_and(t_thread **th, int ***gr)
{
	...
	free(tab);
	return (instr_and_inner(th, gr, size1, size2));
}
